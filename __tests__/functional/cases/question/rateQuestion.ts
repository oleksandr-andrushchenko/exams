...
describe('Rate question', () => {
  ...
  test('Rated', async () => {
    ...
    const nonUserQuestionMark = await framework.fixture<QuestionRatingMark>(QuestionRatingMark, {
    ...
    expect(
      await framework.repo(QuestionRatingMark).countBy({
        questionId: question.id,
        mark,
        creatorId: user.id
      })
    ...
    expect(updatedUser.questionRatingMarks[userAnyQuestionRatingMark.mark - 1][0].toString()).toEqual(
      userAnyQuestionRatingMark.questionId.toString()
    )
