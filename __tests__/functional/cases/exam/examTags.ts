...
describe('Exam tags', () => {
  test('Creates, normalizes, reuses, and attaches tags', async () => {
    ...
    const first = await request(framework.app)
      .post('/')
      .send(
        createExam(
          {
    ...
    const second = await request(framework.app)
      .post('/')
      .send(
        createExam(
          {
  ...
  test('Filters exams by tag slug', async () => {
    ...
    const user = await framework.fixture<User>(User, { permissions: [ExamPermission.Create] })
    const token = (await framework.auth(user)).token
    const tagged = await request(framework.app)
      .post('/')
      .send(
        createExam({
