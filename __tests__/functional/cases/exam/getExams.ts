...
describe('Get exams', () => {
  ...
  test.each([{ size: 1 }, { size: 2 }, { size: 3 }, { size: 4 }])(
    ...
    async ({ size }) => {
      await framework.clear(Exam)
      const exams = (
        await Promise.all([framework.fixture<Exam>(Exam), framework.fixture<Exam>(Exam), framework.fixture<Exam>(Exam)])
  ...
  test('Not empty', async () => {
    await framework.clear(Exam)
    const exams = await Promise.all([framework.fixture<Exam>(Exam), framework.fixture<Exam>(Exam)])

    const res = await request(framework.app)
      .post('/')
      .send(getExams({ size: 50 }, ['id', 'name']))
