...
export default class EntityRepository<Entity extends ObjectLiteral> extends Repository<Entity> {
  public findOneBy(where: any): Promise<Entity | null> { return super.findOne({ where: this.normalize({ ...where, deletedAt: { $exists: false } }) }) }
  public findOne(options: any = {}): Promise<Entity | null> { return super.findOne({ ...options, where: this.normalize({ ...(options.where ?? {}), deletedAt: { $exists: false } }) }) }
  public findBy(where: any): Promise<Entity[]> { return super.find({ where: this.normalize({ ...where, deletedAt: { $exists: false } }) }) }
  public findOneById(id: string | ObjectId): Promise<Entity | null> { return this.findOneBy({ id }) }
  public findByIds(ids: ObjectId[]): Promise<Entity[]> { return this.findBy({ id: { $in: ids } }) }
  public find(options: FindManyOptions<Entity> | any = {}): Promise<Entity[]> {
    ...
    return super.find({ ...(wrapped ? options : {}), where: this.normalize({ ...(wrapped ? options.where : options), deletedAt: { $exists: false } }) })
  }
  public count(query: any = {}): Promise<number> { return super.count({ where: this.normalize({ ...query, deletedAt: { $exists: false } }) }) }
  public countBy(query: any = {}): Promise<number> { return this.count(query) }
  public async sumBy(columnName: keyof Entity, where: any = {}): Promise<number> {
    const result = await this.createQueryBuilder('entity').select(`COALESCE(SUM(entity.${ String(columnName) }), 0)`, 'sum')
      .where(this.normalize({ ...where, deletedAt: { $exists: false } }) as any).getRawOne()
