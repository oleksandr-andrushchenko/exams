...
export class MeResolver {
  ...
  public async createMe(@Arg('createMe') createMe: CreateMe): Promise<User> {
    return await this.meCreator.createMe(createMe)
  }

  @Authorized()
  @Query((_returns) => User, { name: 'me' })
  public async getMe(@Ctx('user') user: User): Promise<User> {
    return user
  }

  @Authorized()
  @Mutation((_returns) => User)
  public async updateMe(@Arg('updateMe') updateMe: UpdateMe, @Ctx('user') user: User): Promise<User> {
    return await this.meUpdater.updateMe(updateMe, user)
  }

  @Authorized()
  @Mutation((_returns) => Boolean)
  public async deleteMe(@Ctx('user') user: User): Promise<boolean> {
