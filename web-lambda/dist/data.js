...
async function getHomeData(size = 8, includeUnapproved = false, includeUnapprovedQuestions = includeUnapproved) {
    const [tags, exams, popularExams, questions, popularQuestions, popularUsers] = await Promise.all([
        ...
        query(`SELECT "id", COALESCE("slug", "id") AS "slug", COALESCE((SELECT u."slug" FROM "users" u WHERE u."id" = "exams"."creatorId"), "creatorId") AS "userSlug", "name", "imageFilename", "questionCount", "approvedQuestionCount", "requiredScore", "rating" FROM "exams" WHERE ${deletedFilter} ${includeUnapproved ? '' : 'AND "ownerId" IS NULL'} ORDER BY "id" DESC LIMIT $1`, [size]),
        query(`SELECT "id", COALESCE("slug", "id") AS "slug", COALESCE((SELECT u."slug" FROM "users" u WHERE u."id" = "exams"."creatorId"), "creatorId") AS "userSlug", "name", "imageFilename", "questionCount", "approvedQuestionCount", "requiredScore", "rating" FROM "exams" WHERE ${deletedFilter} ${includeUnapproved ? '' : 'AND "ownerId" IS NULL'} ORDER BY "rating" DESC NULLS LAST, "id" DESC LIMIT $1`, [size]),
        ...
        query(`SELECT u."id", COALESCE(u."slug", u."id") AS "slug", u."name", u."imageFilename", u."createdAt", u."updatedAt", ${userRatingSelect} FROM "users" u WHERE u."deletedAt" IS NULL ORDER BY u."rating" DESC NULLS LAST, u."id" DESC LIMIT $1`, [size])
    ]);
...
async function getExams(size = 50) {
    return query(`SELECT "id", COALESCE("slug", "id") AS "slug", COALESCE((SELECT u."slug" FROM "users" u WHERE u."id" = "exams"."creatorId"), "creatorId") AS "userSlug", "name", "imageFilename", "questionCount", "approvedQuestionCount", "requiredScore", "rating" FROM "exams" WHERE ${deletedFilter} ORDER BY "id" DESC LIMIT $1`, [size]);
}
async function getExamList(f = {}, includeUnapproved = false) {
    ...
    const rows = await query(`SELECT "id", COALESCE("slug", "id") AS "slug", COALESCE((SELECT u."slug" FROM "users" u WHERE u."id" = "exams"."creatorId"), "creatorId") AS "userSlug", "name", "imageFilename", "questionCount", "approvedQuestionCount", "requiredScore", "rating" FROM "exams" WHERE ${conditions.join(' AND ')} ORDER BY ${column} ${direction(f)} LIMIT $${values.length - 1} OFFSET $${values.length}`, values);
...
async function getExam(examSlug, includeUnapproved = false) {
    const exams = await query(`SELECT "id", COALESCE("slug", "id") AS "slug", COALESCE((SELECT u."slug" FROM "users" u WHERE u."id" = "exams"."creatorId"), "creatorId") AS "userSlug", "name", "imageFilename", "questionCount", "approvedQuestionCount", "requiredScore", "rating" FROM "exams" WHERE ("slug" = $1 OR "id" = $1) AND ${deletedFilter} ${includeUnapproved ? '' : 'AND "ownerId" IS NULL'}`, [examSlug]);
    if (!exams[0])
        return undefined;
...
async function getQuestion(questionSlug, userId, includeUnapproved = false) {
    ...
    if (userId)
        values.push(userId);
    const rows = await query(`SELECT q."id", COALESCE(q."slug", q."id") AS "slug", q."examId", q."title", q."imageFilename", q."difficulty", q."type", COALESCE(e."slug", e."id") AS "examSlug", COALESCE((SELECT u."slug" FROM "users" u WHERE u."id" = e."creatorId"), e."creatorId") AS "userSlug", e."name" AS "examName", q."rating"${userMark} FROM "questions" q LEFT JOIN "exams" e ON e."id" = q."examId"${userJoin} WHERE (q."slug" = $1 OR q."id" = $1) AND q."deletedAt" IS NULL ${includeUnapproved ? '' : 'AND q."ownerId" IS NULL'}`, values);
    if (!rows[0])
        return undefined;
...
async function getUsers(size = 50) {
    return query(`SELECT u."id", COALESCE(u."slug", u."id") AS "slug", u."name", u."imageFilename", u."createdAt", u."updatedAt", ${userRatingSelect} FROM "users" u WHERE u."deletedAt" IS NULL ORDER BY u."id" DESC LIMIT $1`, [size]);
}
async function getUserList(f = {}) {
    ...
    values.push(size + 1, (page - 1) * size);
    const rows = await query(`SELECT u."id", COALESCE(u."slug", u."id") AS "slug", u."name", u."imageFilename", u."createdAt", u."updatedAt", ${userRatingSelect} FROM "users" u WHERE ${conditions.join(' AND ')} ORDER BY ${column} ${direction(f)} LIMIT $${values.length - 1} OFFSET $${values.length}`, values);
    return { data: rows.slice(0, size), page, size, hasNext: rows.length > size };
}
...
async function getUser(userSlug) {
    return (await query(`SELECT u."id", COALESCE(u."slug", u."id") AS "slug", u."name", u."imageFilename", u."email", u."permissions", u."createdAt", u."updatedAt", ${userRatingSelect} FROM "users" u WHERE (u."slug" = $1 OR u."id" = $1) AND u."deletedAt" IS NULL`, [userSlug]))[0];
}
