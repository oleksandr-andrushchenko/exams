...
const Rating = ({ className = '', rating = {}, showAverageMark = false, showMarkCount = false, onChange = () => {
    ...
    return (<div className={`flex items-center gap-2 ${className}`}>
      ...
      <react_2.Rating value={Math.floor(averageMark)} count={5} onChange={(mark) => onChange(mark, setRating, { setLoading, setError })} ratedIcon={<react_bootstrap_icons_1.StarFill className="h-5 w-5"/>} unratedIcon={<react_bootstrap_icons_1.Star className="h-5 w-5"/>} ratedColor={marked ? 'yellow' : 'gray'} unratedColor={marked ? 'yellow' : 'gray'} readonly={readonly || marked}/>
