...
function FormikSelect({ name, label, options, append }) {
    ...
    return (<div className="flex flex-col gap-1">
      <react_1.Select {...input} name={name} label={label || name} onChange={value => setValue(value)} containerProps={{
        ...
        }} defaultValue={value} success={touched && !error} error={touched && !!error} className="capitalize">
        {options.map(option => (<react_1.Option key={option.key || option.value} value={option.value} className="capitalize">
            {option.label}
          </react_1.Option>))}
      </react_1.Select>
