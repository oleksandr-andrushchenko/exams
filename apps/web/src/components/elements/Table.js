...
const Table = ({ className = '', key2, defaultSearchParams = { size: '20' }, queryOptions, queryData, buttons = {}, tabs = {}, filters = {}, columns = [], mapper, }) => {
 ...
 {(Object.keys(tabs).length > 0) && (<div className="d-flex gap-5 align-items-center mt-4">
 {Object.entries(tabs).map(([filter, values]) => (<div key={filter} className="d-flex gap-2 align-items-center">
 <react_1.Typography variant="small">{filter}:</react_1.Typography>
 <react_1.Tabs value={searchParams.get(filter) || 'all'}>
 <react_1.TabsHeader>
 {['all', ...values].map(value => (<react_1.Tab key={value} value={value} className="small small small" onClick={() => applySearchParams({ [filter]: value === 'all' ? undefined : value })}>
 {value}
 </react_1.Tab>))}
 </react_1.TabsHeader>
 </react_1.Tabs>
 ...
 return (<react_1.Select key={`${filter}-${Object.keys(values).join('')}`} label={filter} onChange={value => applySearchParams({ [filter]: value === '' ? undefined : value })} value={searchParams.get(filter) || ''} className=" text-capitalize" containerProps={{ className: '' }}>
 {Object.entries({ '': 'all', ...values }).map(([value, label]) => {
 return (<react_1.Option key={`${filter}-${value}`} value={value} disabled={value === searchParams.get(filter)} className="text-capitalize">
 {String(label)}
 </react_1.Option>);
 })}
 </react_1.Select>);
 ...
 <react_1.Input label="Search" value={searchParams.get('search') || ''} onChange={(e) => applySearchParams({ search: e.target.value === '' ? undefined : e.target.value })} icon={<solid_1.MagnifyingGlassIcon className=""/>}/>

 <react_1.Select label="Size" onChange={(size) => applySearchParams({ size: size === defaultSearchParams['size'] ? undefined : size })} value={searchParams.get('size') || defaultSearchParams['size']} className="text-capitalize">
 ...
 <table className="w-100 table-layout-fixed text-start small mt-4">
 <thead>
 ...
 <Spinner_1.default type="text" width="w-100"/>
 <Spinner_1.default type="text" width="w-100"/>
 <Spinner_1.default type="text" width="w-100"/>
 <Spinner_1.default type="text" width="w-100"/>
 <Spinner_1.default type="text" width="w-100"/>
 ...
 <td colSpan={columns.length} className="p-4 text-center">No data</td>
 </tr>}
