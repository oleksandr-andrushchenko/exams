      ...
      <react_1.Card>
        <react_1.CardBody className="flex flex-col gap-4">
          ...
          <formik_1.Formik initialValues={{
            ...
            {({ values, isSubmitting }) => (<formik_1.Form className="flex flex-col gap-6">
                ...
                {values.type === CreateQuestion_1.QuestionType.CHOICE && (<formik_1.FieldArray name="choices">
                    {({ remove, push }) => (<div className="flex flex-col gap-6">
                        {values.choices.map((_choice, index) => (<div key={`choices.${index}`} className="flex flex-col gap-3">
