...
@Service()
@EventSubscriber(ExamEvent.Approved)
export default class ExamApprovedEventSubscriber implements EventSubscriberInterface {
  public constructor(@Inject() private readonly examActivityCreator: ExamActivityCreator) {}

  public async handle({ exam }: { exam: Exam }): Promise<void> {
    await this.examActivityCreator.createExamActivity(exam, ExamEvent.Approved)
  }
}
