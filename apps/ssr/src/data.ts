...
export async function getExamList(f: SsrListFilters = {}): Promise<SsrPage<SsrExam>> {
  ...
  const rows = await query<SsrExam>(`SELECT "id", "name", "questionCount", "approvedQuestionCount", "requiredScore", "rating" FROM "exams" WHERE ${conditions.join(' AND ')} ORDER BY ${column} ${direction(f)} LIMIT $${values.length - 1} OFFSET $${values.length}`, values)
...
export async function getQuestionList(f: SsrListFilters = {}): Promise<SsrPage<SsrQuestion>> {
  ...
  values.push(size + 1, (page - 1) * size)
  const rows = await query<SsrQuestion & { examName?: string }>(`SELECT q."id", q."examId", q."title", q."difficulty", q."type", e."name" AS "examName", q."rating" FROM "questions" q LEFT JOIN "exams" e ON e."id" = q."examId" WHERE ${conditions.join(' AND ')} ORDER BY ${column} ${direction(f)} LIMIT $${values.length - 1} OFFSET $${values.length}`, values)
...
export async function getUsers(size = 50) { return query<SsrUser>(`SELECT u."id", u."name", u."createdAt", u."updatedAt", ${userRatingSelect} FROM "users" u WHERE u."deletedAt" IS NULL ORDER BY u."id" DESC LIMIT $1`, [size]) }
...
export async function getUserList(f: SsrListFilters = {}): Promise<SsrPage<SsrUser>> {
  ...
  values.push(size + 1, (page - 1) * size)
  const rows = await query<SsrUser>(`SELECT u."id", u."name", u."createdAt", u."updatedAt", ${userRatingSelect} FROM "users" u WHERE ${conditions.join(' AND ')} ORDER BY ${column} ${direction(f)} LIMIT $${values.length - 1} OFFSET $${values.length}`, values)
  return { data: rows.slice(0, size), page, size, hasNext: rows.length > size }
}
